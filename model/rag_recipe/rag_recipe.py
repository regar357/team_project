...
3. title, description을 제외한 모든 텍스트 값은 한국어와 숫자, 기본 특수문자(.,!?() 공백)만 사용한다.
4. title, description을 제외한 필드에는 영어/일본어/중국어/한자/이모지/의미불명 문자를 절대 사용하지 않는다.
5. 맞춤법과 띄어쓰기를 지키고, 자연스럽고 읽기 편한 문장으로 작성한다.
6. 임박 재료는 반드시 실제로 사용하는 재료 목록과 조리 과정에 모두 반영한다.
7. 요리 단계는 구체적이고 따라 하기 쉽게 작성한다.
8. 과장된 표현(“JMT”, “대박”, “존맛” 등)이나 인터넷 용어는 사용하지 않는다.
...
3. title, description을 제외한 모든 텍스트 값은 한국어, 숫자, 기본 특수문자(.,!?() 공백)만 사용한다.
4. 영어, 일본어, 중국어, 한자, 이모지, 의미 불명 문자는 절대 사용하지 않는다.
5. 맞춤법, 띄어쓰기, 문장 어투를 자연스럽게 정리한다.
6. 조리 단계는 논리적인 순서로, 실제로 따라 할 수 있게 구체적으로 작성한다.
7. 임박 재료는 가능한 한 많이 활용하되, 과한 양은 피한다.
8. 레시피의 기본 구조(키 이름)는 유지한다.
